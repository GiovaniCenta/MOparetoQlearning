...
class Pareto():
    ...
    def initializeState(self):
        ...
        #s = self.env.state_to_array(state)
        s = ''.join(str(state))
    ...
    def step(self,state):
        ...
        #next_s = self.env.state_to_array(next_state)
...
if __name__ == '__main__':
    ...
    env = FourRoom()
    numberOfStates = 169*169
    numberOfObjectives = 3
    epsilon = 1
    epsilonDecrease = 0.998
    acMeth = actionMethods(epsilon,epsilonDecrease)
    ref_point = np.array([0, -100,-100])
    gamma = 0.98
